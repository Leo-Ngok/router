...
static void encapsulate_packet(int n_rte_items, int __if_index) {
  if(n_rte_items == 0) return;
  ...
  ether_addr dst_mac = {0x33,0x33, 0x00, 0x00, 0x00, 0x09};
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (time > last_time + 5 * 1000) {
      ...
      for(int if_number = 0; if_number < N_IFACE_ON_BOARD; ++if_number) {
    ...
    if (dst_is_me) {
      ...
      if (ttl <= 1) {
        ...
        HAL_SendIPPacket(if_index, dump, sizeof(ip6_hdr) + ntohs(ip_header->ip6_plen), target);
        delete [] dump;
